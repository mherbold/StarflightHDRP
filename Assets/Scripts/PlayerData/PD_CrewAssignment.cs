...
public class PD_CrewAssignment
{
	...
	public bool IsAssignedToAnyRole( int fileId )
	{
		if ( m_captainFileId == fileId ) return true;
		if ( m_scienceOfficerFileId == fileId ) return true;
		if ( m_navigatorFileId == fileId ) return true;
		if ( m_engineerFileId == fileId ) return true;
		if ( m_communicationsOfficerFileId == fileId ) return true;
		if ( m_doctorFileId == fileId ) return true;
